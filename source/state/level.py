...
class Level(tool.State):
    ...
    def play(self, mouse_pos, mouse_click):
        ...
        if c.AUTO_SUNSELECT and (self.current_time-self.sun_collect_time)>=c.SELECT_SUN_INTERVAL:
            # print(self.sun_group.sprites)
            for sun in self.sun_group:
                self.menubar.increaseSunValue(sun.sun_value)
                self.sun_group.remove(sun)
                self.sun_collect_time = self.current_time
                break
